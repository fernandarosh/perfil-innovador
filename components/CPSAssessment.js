...
const SUPABASE_URL = 'https://ywtdjwkxgvtntaickksbk.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl3dGRqd2t4Z3Z0bnRhaWNrc2JrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk4NjIyNjYsImV4cCI6MjA2NTQzODI2Nn0.OlBckntHtKRGgVAaSpmUtrpxUm3wCMefqftXJd0j-kY';

// Cliente de Supabase simplificado (sin librería externa)
...
const CPSAssessment = () => {
  const [currentStep, setCurrentStep] = useState('register'); // 'register', 'assessment', 'results'
  ...
  const startAssessment = async () => {
    ...
    try {
      ...
      console.log('Datos del usuario que se guardarían:', {
      ...
      console.log('✅ Usuario simulado guardado exitosamente');
  ...
  const calculateResults = async () => {
    ...
    // Calcular porcentajes (fórmula: ((suma)/120)/2)
    const porcentajes = {};
    ...
    // En desarrollo: simular guardado de evaluación
    setIsLoading(true);
    ...
    try {
      ...
      console.log('Datos de evaluación que se guardarían:', evaluationData);
      
      // Simular delay de red
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      console.log('✅ Evaluación simulada guardada exitosamente');
  ...
  const SpiderChart = ({ data }) => {
    const size = 364; // Aumentado 30% (280 * 1.3)
    const center = size / 2;
    const maxRadius = 130; // Aumentado proporcionalmente (100 * 1.3)
    const levels = 5; // 5 niveles concéntricos
    
    const dataArray = Object.entries(data);
    const maxValue = Math.max(...Object.values(data));
    
    // Calcular ángulos para 4 puntos (cuadrantes)
    ...
    // Crear puntos del polígono de datos
        ...
        <div className="relative">
          <svg width={size} height={size} className="overflow-visible">
            ...
            {dataArray.map(([key, value], index) => {
              const angle = index * angleStep;
              const labelPosition = polarToCartesian(angle, maxRadius + 50); // Distancia fija y equitativa para todos
              return (
                <g key={key}>
                  <text
                    x={labelPosition.x}
